...
from init_update_table import init_update_table
...
def update_table(logging, cur, conn):
    try:
        logging.info("Time zone impostata su 'Europe/Rome'.")
        init_update_table(cur, conn)
            ...
            for statement in sql_statements:
                if statement.strip():
                    try:
                        ...
                        logging.info(f"Istruzione eseguita con successo: {statement.strip()[:50]}...")
                    except Exception as e:
                        logging.error(
                            f"Errore durante l'esecuzione dell'istruzione: {statement.strip()[:50]}... Errore: {e}")
                        break
            conn.commit()
    except Exception as e:
